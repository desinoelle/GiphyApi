...
# ***Bugs: 
There is only one "bug" in this webapp that I haven't figured out yet. When the page loads, all of the buttons work fine. But for some reason when the user adds a button and all of the buttons are updated, none of them work anymore. I also couldn't think of a unique category of gifs to use for this project, so I ended up just using animals. I will probably change my category in the future once I solve this buttons bug.
# *Update* Bug has been fixed by changing $(".button").on("click", function()) to $(document).on("click", ".button", function())
# Link to Giphy Api: https://developers.giphy.com/docs/
# Created using HTML, CSS, Javascipt, JQuery, AJAX, and Giphy Api.